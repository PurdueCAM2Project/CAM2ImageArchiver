...
class TestStreamParser(unittest.TestCase):
    ...
    @patch('StreamParser.urlopen', return_value=DummyUrlObject('', None))
    ...
    @patch('StreamParser.urlopen', return_value=DummyUrlObject('Test', None))
    ...
    @patch('StreamParser.urlopen', return_value=DummyUrlObject(None, ['test']))
    ...
    @patch('StreamParser.urlopen', return_value=DummyUrlObject(None, ['--myboundary', 'test']))
    ...
    @patch('StreamParser.urlopen', return_value=DummyUrlObject(None, ['--myboundary', 'Content-Type: image/jpeg', 'first:second:third']))
    ...
    @patch('StreamParser.urlopen', return_value=DummyUrlObject(None, ['--myboundary', 'Content-Type: image/jpeg', 'Content-Length:not_a_digit']))
    def test_mjpeg_frame_line_three_invalid_content_length(self, mocked_urllib):
        self.mjpeg_stream_parser.open_stream()
        self.assertRaises(CorruptedFrameError, self.mjpeg_stream_parser.get_frame)
        self.assertEqual(self.mjpeg_stream_parser.mjpeg_stream.readline_count, 3)

    @patch('StreamParser.urlopen', return_value=DummyUrlObject(None, ['--myboundary', 'Content-Type: image/jpeg', 'wrong:10']))
    def test_mjpeg_frame_line_three_invalid_content_length(self, mocked_urllib):
        self.mjpeg_stream_parser.open_stream()
        self.assertRaises(CorruptedFrameError, self.mjpeg_stream_parser.get_frame)
        self.assertEqual(self.mjpeg_stream_parser.mjpeg_stream.readline_count, 3)

    @patch('StreamParser.urlopen', return_value=DummyUrlObject(None, ['--myboundary', 'Content-Type: image/jpeg', 'Content-Length:10', 'not-empty']))
    ...
    @patch('StreamParser.urlopen', return_value=DummyUrlObject(None, ['--myboundary', 'Content-Type: image/jpeg', 'Content-Length:10', '', '']))
    @patch('StreamParser.cv2.imdecode', return_value=None)
    @patch('StreamParser.np.fromstring', return_value=None)
